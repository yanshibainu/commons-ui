...
const SideBarContainer = forwardRef((props, ref) => {
  ...
  const [sidebarWidth, setSidebarWidth] = useState(showSidebar ? sidebarShowWidth : sidebarHideWidth);
  ...
  const _setShowSidebar = (value) => {
    ...
    if (!value) {
      ...
      setSidebarWidth(sidebarShowWidth);
    else
  ...
  useEffect(() => {
    ...
    sidebarRef.current.parentElement.addEventListener('transitionend', transitionEnd);
    return () => sidebarRef.current.parentElement.removeEventListener('transitionend', transitionEnd);
  }, [])

  return <Container>
    <Sidebar
      style={{ display: 'flex', flexDirection: 'column', flex: `"0 0 ${sidebarWidth}"` }}
      width={sidebarWidth}
      collapsible>
      <div ref={sidebarRef}
        style={{
        ...
        <IconButton
          appearance="link"
          icon={<Icon icon={showSidebar ? angleStyles.angleLeft : angleStyles.angleRight} />}
    ...
    <Content>
      <IconButton
        appearance="link"
        icon={<Icon icon={showContent ? angleStyles.angleRight : angleStyles.angleLeft} />}
